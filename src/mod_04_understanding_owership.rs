...
fn first_word(s:&String)->usize{
    //使用as_bytes()方法将string转化为字节数组
    let bytes = s.as_bytes();
    //在bytes上使用iter创建一个迭代器，iter返回集合中的每一个元素，enumerate方法将元素作为元组返回
...
fn ch_04_enter(){
    ...
    //因为Rust的特性，所以像下边这样在do_something方法后，想要再次使用变量s1将会导致一个编译器错误
    //这是因为s1的值移动到方法内部后，在离开方法时，将会被释放，因此在调用do_something方法后使用s1变量在Rust中是不被允许的
    // let s1 = String::from("hello");
    // println!("s1 is :{s1}");

    
    //为了解决这个问题，可以使用引用，像下面这样
    //从Rust的角度来讲，这并不会转移s1的所有权到方法内部，但却可以访问s1所持有的内存地址和其数据
    //注意一个引用的作用域从声明的地方开始一直持续到最后一次使用为止
    //当变量离开作用域时，rust将自动调用一个叫做drop的特殊方法释放该变量持有的内存
...
//方法的参数传入一个string的引用
//如果想要改变引用的值，则需要加上mut关键字，在变量声明和方法签名处都是如此，这被称为可变引用。
