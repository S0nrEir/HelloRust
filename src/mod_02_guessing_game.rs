...
pub fn enter(){
    ...
    //rand::thread_rng()方法提供一个随机数生成器，它位于当前线程，从系统获取随机种子，接着调用gen_range方法来生成一个指定范围的随机数
    ...
    loop {
        ...
        //通过guess的指定类型（:u32）来告诉parse方法要转换成什么类型
        let guess:u32 = guess.trim().parse().expect("parse faild");

        //cmp方法用来比较两个值，它接受一个引用作为参数，他会返回一个Ordering枚举
