...
pub fn eat_at_restaurant(){
    ...
    //***
    //在rust中，默认的所有项（方法，方法，结构体，枚举等）对于父模块都是私有的
    //父模块不能使用子模块中的私有项，但是子模块可以使用父模块中的项
    //可以使用pub关键字来创建公共项，使子模块的部分项可以暴露给父模块
    //因此使用pub关键字标记hosting模块和add_to_waitlist方法
    //（单独标记hosting模块是不行，因为add_to_waitlist方法仍然是私有的）
    //***
...
pub struct Screen{
    ...
    //Rust 编译器会通过单态化(Monomorphization) 将泛型方法展开.
    ...
    //编译器为每一个被泛型类型参数代替的具体类型生成了方法和方法的非泛型实现。
...
///State定义了所有状态对象的共有行为
trait State {
    //不同于self,&self或者&mut self作为方法第一个参数
    //这个语法意思是：这个方法只可在持有这个类型的Box上调用（如果类型实例被Box包裹，才可以调用该方法？？？）
