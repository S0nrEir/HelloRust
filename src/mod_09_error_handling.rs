...
fn ch_09_01_enter(){
    ...
    //当发生panic时，Rust会回溯stack并且逐个清理每一个方法的数据，这个过程会有很多工作
...
//使用?来对progapating进行简写，这和read_username_from_file方法起到的作用是一样的
//Result值之后的?表示：如果结果是ok，这个表达式将返回ok中的值，程序继续执行
//如果是Err，Err将作为方法的返回值，就像使用了return关键字
//match和?的区别在于。?的错误值被传递给了from方法，它用于将错误的类型转换为当前方法返回值指定的类型，这是一个标准库的trait
//注意，?运算符只能被用于返回值与?作用的值像兼容的方法。
//换句话说，方法返回值为Result<T,E>的方法才能使用?表达式
//同样?表达式也可用于Option<T>
...
//错误传播
//当调用一个可能会失败的方法时，除了在这个方法处理错误外，还可以选择外部调用方知道这个错误并如何处理，
//将错误的处理方式交给外部调用
//从文件读用户名，返回一个Result<String,io::Error>类型
//如果这个方法没有任何错误成功返回，调用方会收到一个包含String的Ok值
...
fn file_unwrap(){
    ...
    // //unwrap()方法，如果Result值成员是OK，则返回OK中的值，如果Result成员是Err，则unwrap方法会自动调用panic，比如下面这样
